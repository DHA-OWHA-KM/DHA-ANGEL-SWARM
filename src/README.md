...
For device context only: **Sempulse Halo (example), CipherOx CRI M1 (reference). BATDOK-J is a separate plausible producer/interface.** ANGEL SWARM has not tested an integration with any real Sempulse Halo, CipherOx CRI M1 or BATDOK-J. This source claims no compatibility, military fielding, FDA status or completed integration. The implemented listener remains receive-only, off by default and limited to the documented, non-ratified medical CoT extension.
