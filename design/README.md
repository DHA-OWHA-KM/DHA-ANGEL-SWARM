...
The Claude Design canvases the console's visual language was extracted from, kept for provenance along with the three JavaScript files they need to open standalone. These are design artefacts, not the application: `app/` is what runs, and the two canvas-era JavaScript files here have both been superseded by the versions under `app/`.
...
| `support.js` | The Claude Design runtime — generated from `dc-runtime/src/*.ts`, not hand-edited. Byte-identical to `app/support.js`, so the canvases open on their own. |
