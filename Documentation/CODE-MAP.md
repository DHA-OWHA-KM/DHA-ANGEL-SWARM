...
| `angel-map.js` | Draws no map. It mounts the whole console in a same-origin iframe and docks that frame over the slot the design draws, so Globe, Theatre, Tactical 2D and Tactical 3D retain their renderer state and bindings while React rewrites the surrounding page; it also servos the run clock as a rate rather than stepping it, and measures the design's columns to keep the map inset in scale. |
...
| `index.html` | The design build: the canvas document carrying every screen's markup and the `text/x-dc` script that drives them. War Game reads the selected operation/scenario and force assumptions, sweeps five levers at 20/30/40 paired battles per setting from seed 1000, supports nominal or observed-flight variability, binds provenance to completed findings, invalidates stale scenario results, and treats cancellation or worker failure as all-or-nothing. Loads the vendored React pair, `support.js`, `js/telemetry.js` and `js/dataproducts.js`, and carries the favicon inline so no `/favicon.ico` request is ever issued. |
| `design.html` | Shipped counterpart of `index.html`, kept under the name the design canvas addresses it by. Verify equality from the current files rather than relying on a copied historical digest. |
...
| `selftest.html` | An offline page that loads `js/sim.js`, `js/optimizer.js`, `js/mc.worker.js` and `angel-engine.js` as plain scripts and checks them with nothing mocked. Coverage includes the reference result, determinism/common random numbers, scenario and lever behavior, War Game cancellation/failure handling, the audit-chain self-check, and adapter/engine agreement. It publishes its live result on `window.__SELFTEST__`; documentation does not copy a total. |
...
| `montecarlo.js` | The confidence pane. Runs the real engine across a pool of Web Workers with both arms of each replication facing an identical casualty stream, so the estimand is the paired within-battle difference in deaths; reports the measured variance reduction from pairing, and states on screen that the interval bounds the seed and not reality. |

### Workers and harnesses

| File | What it does |
|---|---|
| `mc.worker.js` | One replication at a time off the main thread. It `importScripts` the shipped `sim.js` and `optimizer.js` verbatim, answers a load handshake and returns either one complete paired result or an explicit runtime error; the War Game host validates that protocol. War Game gives each setting deterministic seeds from 1000 and the selected scenario/variability. The host terminates the whole pool and discards partial rows on cancellation, load/handshake timeout, protocol mismatch or runtime failure. |
...
| `shell.js` | The analyst-console navigation and page frame, including the destination mapping used by that shell and the 1 Hz repaint of the main column. The primary design shell exposes fourteen destinations; shared aliases must resolve to one canonical owner in each shell. |
...
| `fpcon-state.js` | Owns the persisted FPCON selection shared by the primary and analyst surfaces. Keyboard activation is isolated from mission playback controls, and same-origin tabs synchronize through browser storage events. |
| `nav-shine.js` | Applies the perpetual clipped white shine to ANGEL SWARM brand icons without changing navigation semantics; brand blocks remain links to Command Overview. |
| `page-track.js` / `resupply-track.js` | Resupply Tracking host and deterministic tracker: standalone synthetic commitment fixtures, seek/playback, reroute and exception states, Canvas 2D schematic and Arm B scheduled-push comparison. It does not read or change engine outcomes and emits no outbound event. |
...
| War Game in `index.html` | Five lever sweeps: fleet size, launch points, datalink outage, triage error and responder qualification/mix. Every non-triage lever creates the same altered world for both arms under common random numbers; triage error affects only CURRENT — TRIAGE & PROXIMITY because ANGEL does not consume triage category. Completed output binds scenario, settings, seed range, nominal/observed-flight variability, declared telementoring and ANGEL-only abort/hold differences, paired gap, 95% CI and better/tied/worse counts. |
...
| `page-map.js` | Theatre Map: builds the map destination around four scales — Globe, Theatre, Tactical 2D and Tactical 3D — mounting the live surface through the `DPB` dock. Controls dispatch through `mapScopeNow()`, `zoomAnyMap()`, `fitAnyMap()` and the `MAP_TOOLS_BY_SCOPE` table, which also decides which controls are drawn; the 3D segment is not drawn at all unless `map3dReady()` is true. |
...
Device names in this section have deliberately different roles: **Sempulse Halo (example)** is an example physiological source, **CipherOx CRI M1 (reference)** is reference context for the compensatory-reserve operating principle, and **BATDOK-J** remains a separate plausible producer/interface for the receive path. ANGEL SWARM has not tested integration with a real Sempulse Halo, CipherOx CRI M1 or BATDOK-J and claims no compatibility, military fielding, FDA or other regulatory status for any named relationship, or completed integration.
...
| `app/design.html` | A shipped counterpart of `app/index.html`; verify equality from the current files rather than relying on the historical relationship. |
